...
def loadFile_f():    
    try:
        inputStream = open('data.dat', 'r') # 以讀檔模式開啟檔案
    except FileNotFoundError:
        print('data.dat not found!')
        return
    ...
    while line != '':
        ...
        name = temp[0]
        score = eval(temp[1])
        access(name, score) # 將資料加入到二元搜尋樹
        line = inputStream.readline()
    inputStream.close() # 關閉檔案
...
def saveFile_f(): # 將資料儲存到檔案
    outputStream = open('data.dat', 'w') # 以寫檔模式開啟檔案

    if root == None:
        print('No student record to save!\n')
    else:
        saveByInorder(root, outputStream) # 以中序法儲存資料
        print('File saved successfully!')
    outputStream.close() # 關閉檔案

# 以中序法儲存資料
def saveByInorder(node, outputStream):
    if (node != None):
        saveByInorder(node.llink, outputStream)
        outputStream.write('%s %d\n' % (node.name, node.score))
        saveByInorder(node.rlink, outputStream)
